...
export class ApolloError extends ExtendableError {
  ...
  serialize(): ErrorInfo {
    const { code, field, message, time_thrown, data, _showLocations, _showPath, path, locations } = this;
...
export const formatError = (error, returnNull = false): ErrorInfo => {
  ...
  if (message) {
    originalError.field = message.split(';').length > 1 ? message.split(';')[1].trim().split(/\svalue./)[1] : undefined;
  }
